\documentclass[12pt]{nature}\usepackage{graphicx, color}
\usepackage{alltt}
...
%\subsection{Pathway inference and association with reaction species}

%Pathways were programmatically defined by identifying subsets of the reactions which carried non-zero flux that satisfied two criteria: First, the relative flux across the 25 nutrient conditions for all reactions was perfectly correlated and secondly; using this subset of reactions, weakly connected sets of reactions were identified.  For each pathway, enzymes associated with the involved reactions and the substrate and products of these reactions were regressed on pathway flux to see if a significant association between flux carried and the reaction specie existed.  To account for non-normality of residuals, the significance of association was determined with a bootstrap approach (algorithm \ref{pwbs}).

%\begin{algorithm}[H]\vspace{2mm}
% \KwIn{Relative concentrations of one metabolite or enzyme (S) and relative flux ($\nu$) across $n$ conditions}
% \KwOut{One p-value associating S and $\nu$ using bootstrapping}\vspace{2mm}
 %\Begin{
%Using least-squares regression, solve for $\alpha$ $\&$ $\beta$ in $\nu = \alpha + \beta[S] + \epsilon$\\
%$\hat{\nu} = \alpha + \beta[S]$\\
%$\epsilon^{c} = \epsilon * \sqrt{\sfrac{n}{n - 2}}$
 %}
%\For{$i \leftarrow 1$ \KwTo 10000}{
%$\nu_{i}^{bs} = \hat{y} + \text{sample}[\epsilon^{c}]$\\
%Using least-squares regression, solve for $\alpha_{i}^{bs}$ $\&$ $\beta_{i}^{bs}$ in $\nu_{i}^{bs} = \alpha^{bs} + \beta^{bs}[S] + \epsilon$
%}
%\KwRet{$p = 1 - 2\left|0.5 - \frac{\sum 1\left[\beta^{bs} > 0\right]}{10000}  \right|$}\vspace{2mm}
%\caption{Associating pathway flux and specie abundance}
%\label{pwbs}
%\end{algorithm}
