...
\caption[Michaelis-Menten kinetics is sufficient to relate substrate, product and enzyme concentrations to flux carried for the reaction triose-phosphate isomerase]{Michaelis-Menten kinetics is sufficient to relate substrate, product and enzyme concentrations to flux carried for the reaction triose-phosphate isomerase.  \textbf{A)} Across 25 conditions, we measured the relative concentration the reaction substrate (DHAP: dihydroxyacetone phosphate), product (GAP: glyceraldehyde 3-phosphate) and enzyme (TPI1).  \textbf{B)} The simple Michaelis-Menten reaction form translates the 25 sets of reaction species into predict flux ($V_{rMM}$) based on the value of four inferred kinetic parameters.  Comparing measured flux to the reaction form's prediction indicates that flux through TPI can be explained by a Michaelis-Menten relationship between substrate, product and enzyme.}
\label{fig:TPI}
\end{figure}
...
Metabolite affinities implied by the reaction forms agree with literature estimates \cite{Scheer:2011df}, with substrates and products varying between unsaturated and saturated on a case-by-case basis, but regulators operating at concentrations near their affinity, where the greatest tuning of flux is possible (\hyperref[fig:occupancy]{Figure \ref{fig:occupancy}}). Inferred thermodynamics are consistent with previous estimates of free energy drops across glycolysis \cite{Flamholz:2013io}, and for near-equilibrium reactions, there is an important role for metabolite concentrations shifting the ratio of forward to reverse flux across nutrient conditions (\hyperref[fig:paramaterValues]{Figure \ref{fig:paramaterValues}}).

These 34 instances of regulation encompassed five instances of ``gold standard'' yeast regulation, and four regulators with strong \textit{in vitro} support in yeast, but no demonstrated physiological importance \textcolor{red}{cite cite cite cite}. In addition to yeast-specific regulation, 22 predicted regulators had never been experimentally tested in \textit{S. cerevisiae}. Before embracing the implications of these regulatory mechanisms, we first sought to test whether a subset of non-yeast regulatory predictions could be verified \textit{in vitro}.
...
\subsection{Evaluating the \textit{in vitro} support for predicted regulation.}
...
In order to distinguish regulators with similar quantitative merit, we aimed to estimate the \textit{a priori} probability that each tested regulator is indeed physiologically meaningful in yeast $Pr(Model)$. From the literature reviews and recent primary literature \cite{Jones:1982dn, Sekine:2007ej, Fraenkel:2011wp}, 22 reported physiological regulators of 16 reactions were assembled. This ``gold-standard'' list was initially treated as \textit{a priori} true regulation. We also constructed a list of 186 BRENDA regulators of the 16 gold standard regulation; we assume that the vast majority of regulation in this list is non-physiological and thus all members were initially treated as \textit{a priori} false regulation. 

Using BRENDA, for each regulator, $\textit{i}$, we determined how many times it has been reported in ($sce_{i}$), and non-yeast ($other_{i}$). We also determined the total number of regulators we were testing for the reaction corresponding to \textit{i} ($N_{i}$). Treating gold standard regulation as \textit{a priori} true regulation (p = 1) and BRENDA regulators as \textit{a priori} false regulation (p = 0), logistic regression was used to determine whether literature measures could distinguish true and false regulation (\hyperref[model_logistic_reg]{Equation \ref{model_logistic_reg}}).
